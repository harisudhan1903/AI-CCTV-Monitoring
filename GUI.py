...
class DetectionApp(QMainWindow):
    def __init__(self):
        ...
        upload_button.clicked.connect(self.upload_image_handler)
        upload_item = QListWidgetItem(self.button_list)
        upload_item.setSizeHint(upload_button.sizeHint())
        self.button_list.setItemWidget(upload_item, upload_button)
    ...
    def upload_image_handler(self):
        ...
        if file_path:
            ...
            processed_image = process_frame(image.copy(), start_time=time.time())

            height, width, channels = processed_image.shape
            bytes_per_line = channels * width
            q_image = QImage(processed_image.data, width, height, bytes_per_line, QImage.Format_BGR888)
